...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enableShoot && Input.GetKeyDown(shootKey))
        {
            Shoot();
        }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("Shoot Setup", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();
        fpc.enableShoot = EditorGUILayout.ToggleLeft(new GUIContent("Enable Shoot", "Determines if the player can shoot."), fpc.enableShoot);
        GUI.enabled = fpc.enableShoot;
